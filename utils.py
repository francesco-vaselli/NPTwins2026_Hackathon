...
def make_corner(data, labels=None, color='steelblue', alpha=0.5, bins=50, hist_kwargs=None,
                scatter_kwargs=None):
    """Build corner plot arrays; returns (fig, axes).
    ...
    color : str
    alpha : float
    bins : int  (used for both 1-D histograms and 2-D hexbin)
    """
    data = np.asarray(data)
    n, d = data.shape
    if labels is None:
        labels = [f'x_{i}' for i in range(d)]
    hist_kw    = dict(histtype='step', lw=1.5, density=True, color=color)
    scatter_kw = dict(s=1, alpha=alpha, color=color, rasterized=True)
    if hist_kwargs:
    ...
    for i in range(d):
        for j in range(d):
            ...
            if j > i:
                ...
                continue
            if i == j:
                ax.hist(data[:, i], bins=bins, **hist_kw)
            else:
                ax.scatter(data[:, j], data[:, i], **scatter_kw)
            if i == d - 1:
...
def plot_corner(generated, target, labels=None, title=None, bins=50, alpha=0.4):
    """Overlay corner plots of two datasets (generated vs target).

    Parameters
    ----------
    generated : array-like, shape (N, d)  — model samples (tomato)
    target    : array-like, shape (N, d)  — reference samples (steelblue)
    labels    : list of str
    title     : str, optional
    bins      : int
    alpha     : float for scatter points
    """
    ...
    for i in range(d):
        for j in range(d):
            ...
            if j > i:
                ...
                continue
            if i == j:
                lo = min(target[:, i].min(), generated[:, i].min())
                hi = max(target[:, i].max(), generated[:, i].max())
                b  = np.linspace(lo, hi, bins + 1)
                ax.hist(target[:, i],    bins=b, histtype='step', lw=1.5,
                        density=True, color='steelblue', label='Target')
                ax.hist(generated[:, i], bins=b, histtype='step', lw=1.5,
                        density=True, color='tomato',    label='Generated')
            else:
                ax.scatter(target[:, j],    target[:, i],    s=1,
                           alpha=alpha, color='steelblue', rasterized=True)
                ax.scatter(generated[:, j], generated[:, i], s=1,
                           alpha=alpha, color='tomato',    rasterized=True)
            if i == d - 1:
                ax.set_xlabel(labels[j], fontsize=10)
            else:
                ax.set_xticklabels([])
    ...
    # Legend on top-right diagonal cell
    leg_ax = axes[0, 0]
    leg_ax.legend(fontsize=8, loc='upper right', frameon=False)
